...
const validationRejectForm = (value) => {
   if (!value) {
      return 'Լրացրեք դաշտը';
   } else if (value.length < 4) {
      return 'Դաշտը պետք է պարունակի ավելի քան 4 նիշ';
   } else if (value.length > 50) {
      return 'Դաշտը պետք է պարունակի ոչ ավելի, քան 50 նիշ';
   }
   return '';
};
const validetionReview = (value, action) => {
   if (!value.length) {
      action('Պարտադիր դաշտ');
      return false;
   }
   return true;
};
      ...
      <Card>
         <Box style={{ marginBottom: '20px' }} className={classes.orderSubBlockSpaceBetween2}>
            ...
            <Box className={classes.inLineBlock}>
               ...
               <Typography color={'#808080'} className={classes.inLineStyle} variant={'h4'}>
                  Ծառայությունների ենթակատեգորիաներ
               </Typography>
               <Typography className={classes.inLineStyle} variant={'h2'}>
        ...
        <Box>
         ...
         <Typography style={{ marginBottom: '10px' }} variant={'h2'}>
            Պատվերի կատարման հարմար ժամանակ
         </Typography>
         ...
         {state?.status === 'not confirmed' && (
            <Typography
               ...
               variant="body1">
               Ձեր անձնական պատվերն ուղարկվել է ձեզ <br />
                  ...
                  {showPhoneNumber &&
                     ...
                     state?.users?.phonenumber !== null && (
                        <Button
                           ...
                           <a href={`tel:+${state?.users?.phonenumber}`}>
