   ...
   return (
      <Card sx={{ boxShadow: 2 }}>
         <Box className={classes.orderSubBlockSpaceBetween}>
            <Typography variant={'h2'}>Պրոֆիլի կարգավիճակը</Typography>
         </Box>
         <CustomDivider />
         <Box style={{ display: 'flex', marginBottom: '10px' }}>
            {state?.users?.img_path ? (
               <Avatar src={`${process.env.REACT_APP_IMG_API}${state?.users?.img_path}`} />
            ) : (
               <UserSvg />
            )}
            <Box>
               <div
                  ...
                  <InfoSVG size={8} color={'#8A74EF'} margin={'-5px 0 0 5px'} />
               </div>
               <Box
                  sx={{
                     display: 'flex',
         ...
         <Typography
            ...
            variant={'h4'}
            component="div">
            {state?.users.phone_status !== 'verified' ? <InfoSVG /> : <SuccessSVG />}
            Համարը հաստատված է
         </Typography>
         <Typography
            style={{ marginBottom: '10px', display: 'flex' }}
