...
function QuestionPage() {
   ...
   return (
      <>
         <Container sx={all}>
            <Box sx={boxesHeader}>
               <Typography sx={header1}>Օգնություն մասնագետներին</Typography>
               <Typography sx={header2}>Հարցեր եւ պատասխաններ</Typography>
            </Box>
...
const Question = React.memo(({ data }) => {
   ...
   return (
      <Box sx={{ height: !open ? 150 : '100%', ...boxes }}>
         <Avatar
            sx={boxesImg}
            src={`https://backend.ligauslug.ru/get-file?path=${data.img_path}`}
         />
         <Box sx={textsDiv}>
            <Typography sx={question}>{data.question}</Typography>
            <Typography sx={answer}>
